const sprites = {
  project: {
    id: 'katakana-project',
    width: 751,
    height: 136,
  },
  profile: {
    id: 'katakana-profile',
    width: 136,
    height: 766,
  },
};

export function KatakanaSprites() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" style={{ display: 'none' }}>
      <defs>
        <symbol
          id={sprites.project.id}
          viewBox={`0 0 ${sprites.project.width} ${sprites.project.height}`}
        >
          <path d="M114.7 34c9-.2 16.1-7.2 16.1-16.3 0-9-7.2-16.3-16.2-16.3A16 16 0 0 0 98.4 17H25C10.3 17 6.3 16.7.8 15.8v26c5-.6 8.5-.8 23.8-.9h60.2c-8.2 33.6-34.2 59-72.5 71 9.4 9 13.4 14.5 18.2 24.2 45.8-20.5 70.8-49.5 82.6-95.8l.9-3.7.4-1.5.2-.5.1-.6Zm-.1-24.5c4.4 0 8.2 3.8 8.2 8.2 0 4.3-3.8 8-8.2 8a8.3 8.3 0 0 1-8.2-8c0-4.4 3.7-8.2 8.2-8.2Zm104.5 111v9.7h26a194 194 0 0 1-1-23.8v-73c0-6.8.2-12 .5-17-6.8.6-9 .6-17.9.6h-77.6c-9.5 0-12.4-.1-17.4-.6.4 6 .6 10.4.6 16.6v74a176 176 0 0 1-1.2 23.5h26.2v-10h61.8Zm0-23h-61.8V40h61.8v57.5Zm45.8-68.7c12.8 4.2 28 11.7 41.3 20.6l12.8-22c-10-6-25.5-13.3-41.3-19l-12.8 20.4Zm73-17a102 102 0 0 1 12.7 23.3l14.2-5.1c-3.3-8.2-7.6-16.3-12.7-23L338 11.9Zm-84.3 52.4a174.4 174.4 0 0 1 41.2 22.5l12.3-22.6a205 205 0 0 0-40.2-20.5l-13.3 20.6ZM358.6 5a106 106 0 0 1 12.6 22.7l13.8-5c-4-9.4-8-16.2-12.8-22.3L358.6 5ZM265 134a72 72 0 0 1 10.2-2.5c53.3-9 86-33.9 104.3-79a89.8 89.8 0 0 1-21.2-18c-10.2 31-29.2 52.2-56.1 62.9a135.7 135.7 0 0 1-44.4 9.2l7.2 27.3Zm185.7-68.8h13.9c8.9 0 14.8.3 18.8.8V44c-5.6.6-9.6 1-18.8 1h-50.3c-8.6 0-13-.4-18.6-1v22c6.2-.6 9.2-.8 18.7-.8h13.3v43H406c-10.5 0-14.1-.2-19.5-1v22c7-.6 9.1-.6 19.6-.6h67c10.2 0 14.2.1 19.4.6v-22c-5.7.8-10.2 1-19.4 1h-22.5v-43Zm97.5-62a55 55 0 0 1-6.2 17.3 113.3 113.3 0 0 1-38.3 43.9 70.8 70.8 0 0 1 20.5 17.3 135 135 0 0 0 32.6-36.2l38.3-.4a101.2 101.2 0 0 1-24 40.9 126.1 126.1 0 0 1-48.1 29.4c8 7 12.5 12.6 17.3 21.8a148.7 148.7 0 0 0 53-37.1A152.2 152.2 0 0 0 623 43c2-6.5 2.4-7.4 4.2-10.3l-12-11.8a24 24 0 0 1-10.7 1.6l-36.1.4 2.9-6.1 1.3-3c.4-1 .7-1.8 1-2.2.7-1.9 1-2.3 1.7-3.6l-27.2-5Zm138 130.3a198 198 0 0 1-1-22V74a267 267 0 0 1 51.8 28l14-24.5a305.4 305.4 0 0 0-65.5-30V27.8c0-9.6.1-16.8.7-21h-27.8c.9 7.8 1 9.2 1 20.9v83.8c0 11.3-.3 15.5-.9 22h27.8Z" />
        </symbol>
        <symbol
          id={sprites.profile.id}
          viewBox={`0 0 ${sprites.profile.width} ${sprites.profile.height}`}
        >
          <path d="M33 651.3c-.2-9-7.2-16.1-16.3-16.1-9 0-16.3 7.2-16.3 16.2A16 16 0 0 0 16 667.6V741c0 14.7-.3 18.7-1.2 24.2h26c-.6-5-.8-8.5-.9-23.8v-60.2c33.6 8.2 59 34.2 71 72.5 9-9.4 14.5-13.4 24.2-18.2-20.5-45.8-49.5-70.8-95.8-82.6l-3.7-.9-1.5-.4-.5-.2-.6-.1Zm-24.5.1c0-4.4 3.8-8.2 8.2-8.2a8.2 8.2 0 0 1 0 16.4 8.3 8.3 0 0 1-8.2-8.2Zm111-104.5h9.7v-26c-5.8.7-12.7 1-23.8 1h-73c-6.8 0-12-.2-17-.5.6 6.8.6 9 .6 17.9v77.6c0 9.5-.1 12.4-.6 17.4 6-.4 10.4-.6 16.6-.6h74c10.7 0 15.6.3 23.5 1.2v-26.2h-10v-61.8Zm-23 0v61.8H39v-61.8h57.5Zm-83-143.3c1.3 3.2 1.8 5.8 1.8 10.5v68.4a139 139 0 0 1-1.2 24.7h26c-.6-7.7-.8-10.3-.8-24.2v-59.8a89 89 0 0 1 42.7 24 116.1 116.1 0 0 1 28.2 48.4 75.8 75.8 0 0 1 24.2-18.1c-11.7-25.3-23.2-41.5-39.2-54.9-16.4-13.8-34.4-22.4-61.2-28.9a19.1 19.1 0 0 1-8-3l-12.6 13Zm13.9-106c7.3 4.4 17 14.5 25.8 26.7 12.2 17 21.1 36.2 27.3 58.4a70 70 0 0 1 20.5-12.3 172.2 172.2 0 0 0-17-37.6h30c10.6 0 14.3.3 20 1v-25.6c-5.2.7-10.6.9-20 .9H69.8a160.4 160.4 0 0 0-23.6-26.7c-3.4-3.1-3.4-3.1-5-5l-13.8 20.2ZM83 262.3c-.8-8.8-.9-13.5-.9-29.7v-68.5c0-16.4.1-22.8.9-29.7H55.2c1 9.7 1.3 15.1 1.3 29.5v68.9c0 15-.3 22.7-1.3 29.5H83ZM8.8 114c3.7-.4 7-.6 13.2-.6 37.3 0 55.2 2.3 69.3 9 9.5 4.6 15.6 9 23 17.2a78.1 78.1 0 0 1 20-20.6 95.8 95.8 0 0 0-28.7-20c-15.7-6.9-31.7-9.8-61.8-10.6-28.5-1-28.5-1-33.4-1.6L8.8 114Zm-.7-41a90 90 0 0 1 11.2-.6h5.2l85.2.7c6 0 8.4.3 12.1 1.6l13-12.4c-1.2-1.8-1.6-3-3-7A86.2 86.2 0 0 0 115.4 25 88.2 88.2 0 0 0 78.8.2a98.4 98.4 0 0 1-19 19.2 73 73 0 0 1 24.6 11.2 55 55 0 0 1 15.8 17.3l-75.4-.7A103 103 0 0 1 8 46v27.1Z" />
        </symbol>
      </defs>
    </svg>
  );
}

export function Katakana({ type, ...rest }) {
  return (
    <svg
      aria-hidden="true"
      width={sprites[type].width}
      height={sprites[type].height}
      {...rest}
    >
      <use href={`#${sprites[type].id}`} />
    </svg>
  );
}

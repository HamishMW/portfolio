...
function Monogram({ className, color, highlight, ...props }) {
  const colorText = props.theme.colorText(1);

  return (
    <svg className={className} fill={color || colorText} width="45" height="29" viewBox="0 0 45 29">
      <defs>
      ...
      <rect clipPath="url(#monogram-clip)" fill={color || colorText} width="100%" height="100%" />
      {highlight &&
        <g clipPath="url(#monogram-clip)">
          <rect fill={highlight || colorText} width="100%" height="100%" />
        </g>
      }
    </svg>
  );
...
export default withTheme(Monogram);
